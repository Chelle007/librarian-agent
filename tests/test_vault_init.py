...
def test_init_creates_structure_and_schema(tmp_path):
    ...
    schema_file = root / "system" / "schema.json"
...
def test_init_is_idempotent_and_preserves_existing_schema(tmp_path):
    root = tmp_path / "vault"
    init_vault(root)
    (root / "system" / "schema.json").write_text('{"custom": true}', encoding="utf-8")

    init_vault(root)  # second run must not clobber
    assert '"custom"' in (root / "system" / "schema.json").read_text(encoding="utf-8")
